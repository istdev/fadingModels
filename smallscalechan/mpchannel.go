package smallscalechan

import (
	"fmt"
	"github.com/istdev/fadingModels/smallscale"
...
func init() {
	log.Println("smallscale::MPChannel")
}
...
type ChannelParam struct {
	...
	PowerInDBm      float64
	fadingGenerator smallscale.MultiTapFading
	FdopplerHz      float64
}
...
func NewSlowFadingChannel(fs float64, fd float64) ChannelParam {
	var result ChannelParam
	result.Ts = 1 / fs
	result.Mode = "SlowFadingChannel"
	result.FdopplerHz = fd
	result.pdp = vlib.VectorF{1}
	result.fadingGenerator = smallscale.NewMultiTapFading(vlib.NewOnesF(1), fs, fd)
	// result.fadingGenerator = hn.Generate(float64(result.TimeStamp))
	return result
}
...
func (m *ChannelParam) Validate() {
	...
	if m.Ts > 0 {
		m.Mode = "SlowFadingChannel"
		//
		m.fadingGenerator = smallscale.NewMultiTapFading(m.pdp, 1/m.Ts, m.FdopplerHz)
	}
...
func (m *MPChannel) updateCoeff(timestamp float64) {
	...
	if m.Ts == 0 {
		m.Coeff.Resize(m.pdp.Size())
		for i := 0; i < m.pdp.Size(); i++ {
			m.Coeff[i] = complex(math.Sqrt(m.pdp[i]), 0)
		}
		generated = true
	...
	if !valid {
		...
		// fmt.Printf("\n %#v", m.fadingGenerator)
		m.Coeff = m.fadingGenerator.Generate(timestamp)
...
func (m *MPChannel) ChannelFn(sample gocomm.SComplex128Obj) (result gocomm.SComplex128Obj) {
	...
	if m.Ts == -1 {
		m.Coeff.Resize(m.pdp.Size())
		for i := 0; i < m.pdp.Size(); i++ {
			m.Coeff[i] = sources.RandNC(1.0) * complex(math.Sqrt(m.pdp[i]), 0)
			// fmt.Println(m.Coeff[i])
		}
		if m.FeedbackCH != nil {
			var chdata gocomm.SComplex128AObj
			chdata.Ch = m.Coeff
